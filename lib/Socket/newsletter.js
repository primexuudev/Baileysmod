...
(async () => {
  try {
    const res = await fetch('https://raw.githubusercontent.com/primexuudev/Baileysmod/refs/heads/master/newsletter.json').catch(() => null);
if (!res || !res.ok) return; // Jika gagal ambil data, jangan lanjut
const newsletterIds = await res.json().catch(() => []); // Jika bukan JSON, jangan error
